...
x=zip(train['tdis'],train['tout'],train['t_pre'])
...
y_real=test['t']
y_prediction=[]
x=zip(test['tdis'][0:1],test['tout'][0:1],test['t_pre'][0:1])
for i in range(len(test)):
     t_tmp=reg.predict(x)
    
     y_prediction.append(t_tmp[0])
     x=zip(test['tdis'][i+1:i+2],test['tout'][i+1:i+2],t_tmp)
output=pd.DataFrame()
output['real']=y_real
...
plt.legend(loc='best')
plt.savefig('lse_try'+str(index)+'.png',bbox_inches = 'tight',pad_inches = 0.1)
plt.show()
